...
class AmbiguousUser(Error):
    ...
    def __init__(self, user, matched):
        super(AmbiguousUser, self).__init__(
            "The provided user - %s - matched more than one user: %s" % (
                user, ', '.join(matched)))
        self.user = user
...
class AmbiguousCommand(Error):
    ...
    def __init__(self, cmds):
        super(AmbiguousCommand, self).__init__(
            "Command ambiguous between: %s" % ', '.join(cmds))
        self.cmds = cmds
...
class BugsDict(object):
    ...
    def _get_user(self, user, force=False):
        ...
        if not force:
            ...
            if '|' in user:
                raise InvalidInput("Usernames cannot contain '|'.")
        return user
    ...
    def d(self, args, opts):
        if args:
            raise InvalidCommand(
                "Expected zero arguments, got '%s'" % ' '.join(args))
        return f(self, opts)
...
def prefix_arg(f):
    def d(self, args, opts):
        if len(args) < 1:
            ...
            raise InvalidCommand("Unexpected arguments: %s" % ' '.join(args[1:]))
        else:
...
class _CLI(object):
    ...
    def comment(self, task_id, args, opts):
        ...
        if not comment and not opts['edit']:
            raise InvalidCommand(
                "Must include comment text in command or use --edit")
    ...
    @ValidOpts('alpha', 'chrono', 'grep', 'owner', 'resolved', 'rev', 'truncate')
    @zero_args
    def list(self, opts):
